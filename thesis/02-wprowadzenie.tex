...
Pierwszą taką cechą jest weryfikacja mierzalnej wartości. \fixme{Każdy test powstaje w
celu sprawdzenia czy oczekiwane przewidywania są spełnione. \emph{- zdanie bez sensu}} Wymagane jest więc
...
sprawdza się w \fixme{ekstremalnych} warunkach itp. W zastosowaniach związanych z
...
eliminacja nie zaburzyła innych komponentów. To samo dotyczy \fixme{nowej}
...
Ponieważ w środowiskach rozproszonych powszechna jest \fixme{sytuacja}, w
której \fixme{mamy do czynienia} ze sprzętem różnego rodzaju i różnych
producentów, a budowane rozwiązania mają niezawodnie pracować przez wiele lat,
testowanie tworzonych produktów jest \fixme{tu} bardzo istotne. Jednocześnie
jest ono trudniejsze niż w wypadku rozwiązań przeznaczonych na
samodzielne\footnote{ang. standalone} systemy.

\FIXME{Ten akapit nie pasuje na koniec. Napisać lepszy na koniec, a ten
przenieść gdzieś indziej. Poza tym dużo niezręcznych słów.}

\section{Testy aplikacji sieciowych}

\fixme{W sieciach komputerowych oprogramowanie występuje wszędzie. -
\emph{filozoficznie pojechałeś:)}} Na różnych warstwach modelu OSI powierza się
...
\FIXME{Trudność polega na poprawnym zdefiniowaniu testu. O ile w przypadku obserwacji
...
\emph{Tu coś miało być źle z tym, że z powyższego nie wynika, żeby to była
trudność. To raz, a dwa to jest bardzo dyskusyjne czy nie można testów
wydajności wygenerować. Ogólnie to jedne można, drugie nie, ale na każdym
poziomie można jakieś tam testy wygenerować.}}
...
chcemy \fixme{opóźnień} związanych z realną transmisją. Możemy nawet uruchomić algorytm
osobno, poza stosem sieciowym, i patrząc \fixme{z boku} analizować jego działanie.
...
\fixme{Obecnie moc obliczeniowa pozwala symulować wiele czynników występujących
w prawdziwych sieciach. - \emph{nie pamiętam co było nie tak, ale to zdanie
śmierdzi.}} Można więc coraz więcej testów przeprowadzać w wirtualnych
...
\FIXME{Mimo wszystko należy myśleć o tych problemach jak o przeszkodach.
Napisać to jakoś inaczej.}

\section{Dostępne rozwiązania}

\FIXME{Ten dział ma być wcześniej.}
...
\FIXME{Jak już używać niespolszczonych nazw to ich nie odmieniać. Tutaj
frameworków, gdzieś indziej switche.}

Istnieje szereg \fixme{frameworków} zbudowanych na bazie środowiska OMNeT++, a które
...
W tabeli \fixref{referencja} porównujemy naszym zdaniem najistotniejsze cechy
wspomnianych narzędzi. \fixme{Jak widać Arete nie jest pozbawione wad, tym
samym nie jest to idealne narzędzie do każdego typu zastosowań. - \emph{LOL,
napisać to mniej zuchwale:)}} Nie jest to również narzędzie zdolne do
