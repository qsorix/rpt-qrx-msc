...
\begin{itemize}
  \item{wymienianych Protocol Data Unit\footnote{\fixme{Czy wszędzie indziej też
pisać rozwinięty skrót?}},}
...
celu zweryfikowanie, czy zbudowane PDU jest zgodne ze
specyfikacją\footnote{\fixme{Coś tutaj było nie tak z tym zdaniem, ale nie umiem
wymyslić co.}}. Od wykorzystywanych tu narzędzi wymaga się, aby umożliwiały
...
położenie bitów w pakiecie) określone są w tym samym miejscu\footnote{\fixme{Co
znaczy jednocześnie?}}. Korzystanie z ASN.1 w takich wypadkach jest
...
OMNeT++ to biblioteka i framework\footnote{\fixme{Postanowiliśmy zdecydowanie
...
\FIXME{
Tutaj ma powstać tabelka w której będą w pionie różne aspekty testowania, a w
poziomie istniejące rozwiązania. W środku + i - czy dany program się nadaje do
tego. Zrobić to tak, żeby wśród rozwiązań był nasz program i wszędzie miał +.
Będzie ciężko, bo nasz program też ma dość specyficzne zastosowanie. Pomyśleć
jakby to zrobić, żeby mimo wszystko wypadł w porównaniu dobrze.

Tabelkę zrobiłam byle jak, żeby było. Bardziej się na texie znasz to zrób to
ładnie.
}

\begin{center}
   \begin{tabular}{ r | c | c | c | c }
       & ASN.1+ECN & TTCN & OMNeT++ & NAZWA \\
      \hline
      cecha1 & + & + & - & + \\
      \hline
      cecha2 & - & + & + & + \\
