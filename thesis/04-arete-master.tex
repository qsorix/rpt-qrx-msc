...
Następnie rozważaliśmy stworzenie dedykowanego języka (\fixme{DSL}), którego
cechy będą odpowiadać naszym wymaganiom. Inspiracją był tutaj język
NED\footnote{NEtwork Description} używany przez symulator sieci
...
implementacji modułów w języku C++, my postanowiliśmy skorzystać z języka
Python.

\FIXME{Jakoś średnio jest podkreślony nasz długi proces wyboru języka.}

\fixme{Ostatecznie} zdecydowaliśmy użyć języka Python jako języka konfiguracji. Tego typu
...
\item model,
\item laboratorium,
\item mapowanie \fixme{- może mapowanie, żeby było bardziej po polsku},
\item plan.
\end{itemize}
...
którym w rzeczywistości dysponujemy. \fixme{Mapowanie} zapewnia odwzorowanie urządzeń
...
diagramie poniżej. \fixref{ref do diagramu}.
...
\item \code{frontend} -- mówi, który \fixme{plugin} określa sposób w jaki przebiega
komunikacja z danym urządzeniem (protokół przesyłanych komunikatów).
\end{itemize}

\FIXME{I znowu trzeba znaleść lepsze słowo dla pluginów, bo się nie podoba.}

Więcej na temat \fixme{pluginów} piszemy w rozdziale \fixref{referencja do rozdziału o
pluginach}.
...
ten temat w rozdziale o mapowaniu. \fixref{ref do rozdziału o mapowaniu}
...
Stworzone mapowanie podsumowuje diagram \fixref{ref do rysunku}.
...
zastosowania \fixref{ref do rozdziału}. Ich lektura pozwoli lepiej zrozumieć
