...
Więcej na temat pluginów piszemy w rozdziale \fixref{referencja do rozdziału o
pluginach}.
...
\FIXME {
Różnica między HostDriverPlugin i InterfaceDriverPlugin.

Prosty przykład.

Zaznaczyć, że to co w host ma być konfigurowane, a to co w device jest teraz i
służy realizacji. Np. host.ip mówi, jakie IP ustawić. device.ip mówi, pod jakim
IP można się z urządzeniem w tym momencie połączyć.

Omówienie grup komend (check, setup, task, clean) -- btw, to dokładnie powinno
być omówione wcześniej. W opisie architektury systemu.
}
...
Copy-paste klasę File i omówić kod.

Odesłać do metod w reference, które dotyczą zasobów. add\_resource, ale też
wszystkich miejsc, gdzie można powiedzieć, że zasób jest potrzebny (chyba tylko
Command i Host).
