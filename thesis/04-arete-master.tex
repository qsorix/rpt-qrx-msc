...
Rozdział rozpoczynamy przedstawieniem architektury modułu i wprowadzając nazwy
...
\label{arete-master-architektura}
...
rozdziału, tutaj zostanie przedstawiona tylko rola poszczególnych komponentów.
...
urządzeń. Wtyczki te są szerzej omówione w sekcji \ref{arete-master-sterowniki-parametrow}.
...
przed rozpoczęciem wykonywania testu. W sekcji \ref{arete-master-wtyczki-typu-hook}
...
\label{arete-master-konfiguracja}
...
W skład najprostszego testu wchodzi jeden komputer\footnote{Przykładowo możemy
zdecydowań, że zarówno aplikacja serwera jak i klienta będą wykonywać się na
tym samym systemie}. Złożone konfiguracje mogą zawierać ich kilkadziesiąt i
definiować specyficzne sposoby komunikacji, jak np.
Chord\footnote{http://en.wikipedia.org/wiki/Chord\_\%28peer-to-peer\%29}.
...
\label{arete-master-format-konfiguracji}
...
framework. Dokładną dokumentacją zamieszczamy w dodatku
\ref{reference:interfejs-programowy}.

\subsubsection{Model}
\label{arete-master-model}
...
sterowników (patrz sekcja \ref{arete-master-sterowniki-parametrow}), które można
...
funkcji \code{add\_host}. Pierwszym i obowiązkowym argumentem jest nazwa
...
\begin{center}
\leavevmode
\includegraphics[width=0.65\textwidth]{konfiguracja-model}
\end{center}
...
\label{arete-master-laboratorium}
...
\begin{center}
\leavevmode
\includegraphics[width=0.70\textwidth]{konfiguracja-laboratory}
\end{center}
...
\begin{itemize}
\item \code{connection} -- określa, z której wtyczki korzystać w celu
nawiązania komunikacji z danym urządzeniem,
\item \code{frontend} -- mówi, która wtyczka określa sposób w jaki przebiega
...
atrybuty, z których korzysta wtyczka \code{tcp} i w przypadku ich braku,
zgłosiłaby błąd w czasie przetwarzania konfiguracji. Opis wymaganych atrybutów
...
\label{arete-master-odwzorowanie}
...
\begin{center}
\leavevmode
\includegraphics[width=0.71\textwidth]{konfiguracja-mapping}
\end{center}
...
\code{RunPolicy}. Użytkownik może definiować własne strategie -- patrz dodatek
\ref{reference:config.Schedule.RunPolicy}.
...
Inne przykłady definicji planów można znaleźć w rozdziale
\ref{demonstracja-arete} prezentującym przykładowe zastosowania. Ich lektura
pozwoli lepiej zrozumieć w jaki sposób wykorzystać dostępny interfejs.

\subsubsection{Pozostałe}
\label{arete-master-pozostale}
...
Demonstracja zastosowania dostępna jest w rozdziale \ref{demonstracja-arete}.
...
\code{get\_secure} różni się tylko tym, że w przypadku kiedy program poprosi
użytkownika o podanie wartości, zostanie użyty tryb, w którym wprowadzany tekst
...
\label{arete-master-podsumowanie}
...
\label{arete-master-czesc-wykonawcza-frameworku}
...
\label{arete-master-komunikacja-z-modulem-slave}
...
Odpowiadają za to wtyczki typu \code{ConnectionPlugin}. Frontend nie musi
...
\begin{center}
\leavevmode
\includegraphics[width=0.8\textwidth]{arete-master-controller-frontend-sequence}
\end{center}
...
\code{check\_test\_end} kontroler pyta frontendy czy test na podległym
...
\code{Arete\-Slave\-Frontend} -- standardowo dostępnej wtyczki typu frontend.

\subsubsection{AreteSlaveFrontend}
\label{arete-master-areteslavefrontend}

Jest to standardowo dostępna wtyczka typu frontend służąca komunikacji z
modułem Arete Slave opisanym szczegółowo w kolejnym rozdziale.
...
algorytm synchronizacji wtyczki \code{Arete\-Slave\-Plugin}. Dokładność uzyskiwana
przy użyciu NTP jest zależna od wersji protokołu, zastosowanego sprzętu,
systemu operacyjnego i rodzaju sieci \cite{ntp-research}. Może ona wynosić od
dziesiątek milisekund w sieciach WAN do nawet 20 µs w sieciach LAN przy
zastosowaniu odbiorników GPS lub innych dokładnych źródeł czasu. Trzeba też
pamiętać, że wpływ ma tutaj także planista systemowy, który może powodować
opóźnienia w wykonywaniu programów.

\subsection{Połączenie ze zdalnym urządzeniem}
\label{arete-master-polaczenie-ze-zdalnym-urzadzeniem}
...
\label{arete-master-tcpconnection}
...
\label{arete-master-sshconnection}
...
Na marginesie dodamy, że widoczna w przykładzie funkcja \code{relative}
...
\label{arete-master-sterowniki-parametrow}
...
określenie atrybutu \code{ip} powinno owocować wykonaniem komend, które na
...
Dokładne wyjaśnienie znaczenia każdej funkcji znajduje się w dodatku
\ref{reference:interfejs-programowy}.  Tutaj dodamy jeszcze tylko, że sterowniki
dzielą się na te przetwarzające argumenty hostów i te, które przetwarzają
argumenty interfejsów.  Wtyczki dziedziczą odpowiednio z klas
\code{HostDriverPlugin} i \code{InterfaceDriverPlugin}.

\subsection{Przesyłanie zasobów}
\label{arete-master-przesylanie-zasobow}
...
Metoda ta w argumentach otrzymuje instancję klasy \code{HostCommands}, do której
...
Szersze omówienie interfejsu \code{Resource} znajduje się w dodatku
\ref{reference:interfejs-programowy}.

\subsection{Wtyczki typu Hook}
\label{arete-master-wtyczki-typu-hook}

Wtyczki typu Hook umożliwiają rozszerzenie przedstawionego przez nas schematu
konfiguracji o własne algorytmy modyfikujące tworzoną definicję test. Aktualnie
dostępny jest tylko jeden hook: wykonywany po przygotowaniu konfiguracji, ale
jeszcze przed uruchomieniem testu.
...
\label{arete-master-przechowywanie-wynikow}
...
\begin{center}
\leavevmode
\includegraphics[width=0.7\textwidth]{arete-master-db}
\end{center}
...
Główna tabela \code{tests} zawiera informacje dotyczące pojedynczego wykonania
...
\label{arete-master-uruchamianie}
...
muszą być dostępne w wybranych katalogach wtyczek (opcja \code{--plugins})

\item[\code{--map}]

Definiuje odwzorowania z poziomu linii poleceń. W takim wypadku nie należy
podawać go w konfiguracji testu. Można podać wiele odwzorowań w formacie
