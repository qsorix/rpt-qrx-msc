...
\paragraph{Poker} realizuje funkcję \emph{poke}\footnote{\fixme{...?}}, będącą
częścią mechanizmu powiadomień (Rozdział \fixref{Mechanizm Powiadomień}).
Pozwala on na wysyłanie powiadomień w dowolnym momencie testu przez uruchamiane
w jego ramach komendy. Korzysta się z niego przez \code{@\{poke <nazwa
powiadomienia>\}}.
...
przypadku korzystania z mechanizmu powiadomień (Rozdział \fixref{Mechanizm
Powiadomień}).}
...
Pozwala określić port na którym uruchomiony zostanie Arete Slave.

\item[\code{--database}]

Określa plik w którym przechowywana będzie baza danych.

\item[\code{--log}]

Określa plik w którym przechowywany będzie log.

\item[\code{--workdir}]

Określa tymczasowy katalog roboczy dla wykonywanych testów.
...
Powoduje usunięcie bierzącej bazy danych oraz loga.

\item[\code{--clean}]

Powoduje usunięcie katalogu z plikami tymczasowymi.

\item[\code{--ssh}]

Włącza obsługę SSH. Wymagane jest podanie dwóch kolejnych parametrów.
